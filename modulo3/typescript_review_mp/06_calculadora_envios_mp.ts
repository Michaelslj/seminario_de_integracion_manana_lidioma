...
function evaluarInventari(producto: ProductoInventario): string {
  ...
  return `
[${producto.codigo}] Producto    : ${producto.nombre}
Categoría   : ${producto.categoria}
Stock Actual: ${producto.stockActual} unidades (Mín: ${producto.stockMinimo})
Valor Stock : $${valorTotalStock.toFixed(2)}
Almacenaje  : $${costoAlmacenaje.toFixed(2)}
Estado      : ${requiereReabastecimiento ? "⚠️ REABASTECER URGENTE" : "✅ Stock Óptimo"}
─────────────────────────────────────────`.trim();
}

const product1: ProductoInventario = {
  codigo: "ELEC-001",
...
const product2: ProductoInventario = {
  codigo: "PER-045",
